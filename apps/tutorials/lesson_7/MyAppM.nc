 ...
 *
 * $Id: MyAppM.nc,v 1.1.2.3 2007/04/26 20:04:35 njain Exp $
 */
...
implementation {
  ...
  TOS_Msg msg_buffer;
  XDataMsg *pack;
  norace uint16_t	lightData[PhotoLogCount];
  /**
 ...
 /**
  ...
  async event result_t Light.dataReady(uint16_t data) {
	...
	lightData[0]=data;	
	if(call ReadData.read(0,(uint8_t*)(&lightData[1]),(PhotoLogCount-1)*2)==FAIL)
	{
	call Leds.redToggle();
	} 
  ...
  {
  	memcpy((uint8_t*)(pack->xData.datap1.light)+2,(uint8_t*)(&lightData[1]),(PhotoLogCount-1)*2);
  	call WriteData.write(0,(uint8_t*)(&lightData[0]),PhotoLogCount*2);
  }	
  return SUCCESS;
