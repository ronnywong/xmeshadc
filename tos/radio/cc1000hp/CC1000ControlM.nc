 ...
 *
 * $Id: CC1000ControlM.nc,v 1.5.2.1 2007/04/27 04:53:41 njain Exp $
 */

/*
 *
 * Authors:		Jaein Jeong, Phil Buonadonna
 * Date last modified:  $Revision: 1.5.2.1 $
 *
...
//    call CC1000Control.RxMode();
   ...
   *************************************************************************/
   command result_t RadioControl.TunePreset(uint8_t freq) {
	   return call CC1000Control.TunePreset(freq);
   }

   command uint32_t RadioControl.TuneManual(uint32_t DesiredFreq) {
	   return call CC1000Control.TuneManual(DesiredFreq);
   }
