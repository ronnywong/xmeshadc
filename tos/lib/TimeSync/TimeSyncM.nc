 ...
 *
 * $Id: TimeSyncM.nc,v 1.2.4.3 2007/04/27 04:52:27 njain Exp $
 */
...
typedef struct TimeSyncMsg {
  ...
  enum {
    //TRANS_TIME = 16,		Hardcoded for MICA2 CC1000
    //TRANS_TIME = 2,		Hardcoded for MICAZ CC2420
    INTERVAL = 5 * 1024,
    //UPDATE_INTERVAL = 5 * 1000,
    UPDATE_INTERVAL = 5 * 1024,
    SMOOTH_FACTOR = 3,
#ifdef TEN_X
  ...
  tos_time_t new_time;  //moved time update to a task
  //bool updatePending;	// flag for signaling new time
  ...
  command result_t StdControl.start() {
    ...
    call UpdateTimer.start(TIMER_REPEAT, UPDATE_INTERVAL);
    call Timer.start(TIMER_REPEAT, INTERVAL);
    
    //call Timer128.start(TIMER_REPEAT, 128);	//debugging
 ...
 // set the new time here to avoid race condition with periodic updates
 task void setNewTime() {
	 //call TimeSet.set(new_time);	 
	 atomic {
      time.high32 = new_time.high32;
      time.low32 = new_time.low32;
     }
 }
 
 // Incorporated Change by Satoru
 void FillTimeAsync(){
		if(fill_pending == 1){
 ...
 task void SendTask(){
  ...
  event TOS_MsgPtr ReceiveMsg.receive(TOS_MsgPtr pMsg) {
	...
	TimeSyncMsg* tMsg = (TimeSyncMsg*)pMsg->data;
    	...
    	tos_time_t val;
    	//tos_time_t new_time;  Make this global
	...
	call Timer.start(TIMER_REPEAT, INTERVAL); //restart timer
	post setNewTime();  //if this post fails, it would be like not getting an update
#endif
  ...
  async event void RadioCoordinator.startSymbol(uint8_t bitsPerBlock, uint8_t offset, TOS_MsgPtr msgBuff){
	...
	// Incorporated Changes by Satoru
	if(msgBuff == &msg) FillTimeAsync();
	return;
  }
