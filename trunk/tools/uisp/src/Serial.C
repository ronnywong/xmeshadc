// $Id: Serial.C,v 1.1.4.3 2007/01/26 06:05:37 pipeng Exp $
...
void TSerial::SendOnly(unsigned char* queue, int queue_size)
{
  ...
  if (!remote) {
    ...
    usleep(100000);		// Should be more than enough (response
...
void TSerial::OpenPort() { 
 ...
 /* Open port and set serial attributes */
  ...
  if (strncasecmp(dev_name, "com", 3) == 0)
    {
	  char *new_name = new char[13];
      tmp_name=atoi(dev_name+3);
      sprintf(new_name, "/dev/ttyS%d", tmp_name - 1);                
