// $Id: Main.C,v 1.3.2.1 2007/01/26 06:05:15 pipeng Exp $
...
"(c) 2003-2006 MoteWorks Team, Crossbow Technology\n"
...
"             [-dprog=avr910|pavr|stk500|mib510|mib520|stargate]"
#ifndef NO_DAPA
" [-dprog=type]\n"
...
"         mib520    Crossbow MIB520 (for Atmega128, 115200 baud serial only) \n"
...
#ifdef __WIN32__
void xpl_util_sleep(unsigned long ms)
{
	Sleep(ms);
	return;
}
int xpl_util_gettimeofday(struct timeval* tp, void* tzp)
{
	time_t t;
	t = time(0);
    tp->tv_sec = (long)t;
...
int xpl_util_gettimeofday(struct timeval* tp, void* tzp)
{
	return 0;//gettimeofday(tp, (timezone*)tzp);
}
#endif
...
int main(int _argc, const char* _argv[]){
  ...
  try{
    ...
    else if ( (val && (strcmp(val, "stk500") == 0)) || (val && (strcmp(val, "mib510") == 0)) || (val && (strcmp(val, "mib520") == 0))  ) {
	#ifndef __WIN32__
      setgid(getgid());
      setuid(getuid());
	#endif
