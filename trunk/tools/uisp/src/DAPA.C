// $Id: DAPA.C,v 1.1.4.1 2007/01/26 06:05:00 pipeng Exp $
...
TDAPA::Delay_usec(long t)
{
  ...
  xpl_util_gettimeofday(&t1, NULL);
  if (t > MIN_SLEEP_USEC)
    xpl_util_sleep(t - MIN_SLEEP_USEC);//pp
  /* loop for the remaining time */
  t2.tv_sec = t / 1000000UL;
  t2.tv_usec = t % 1000000UL;
  timeradd(&t1, &t2, &t1);
  do {
    xpl_util_gettimeofday(&t2, NULL);
...
TDAPA::ParportSetDir(int dir)
{
  ...
  if (ppdev_fd !=XPL_INVALID_HANDLE) {
#ifdef par_set_dir
...
TDAPA::TDAPA(): 
  parport_base(0x378), ppdev_fd(XPL_INVALID_HANDLE)
{
  const char *val;
  ...
  if (ppdev_name) {
    ...
    if (ppdev_name) {
    if (pa_type_is_serial) {
      ppdev_fd = CreateFile( ppdev_name,GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL|FILE_FLAG_OVERLAPPED, NULL);
		if ( (int)ppdev_fd <= 0 ){
			ppdev_fd = XPL_INVALID_HANDLE;
			throw Error_Device("Failed to claim ppdev.");
		}
	...
	{
      ppdev_fd =(XPL_HANDLE)open(ppdev_name, O_RDWR, 0);
	}
    if (ppdev_fd >= 0) {
      perror(ppdev_name);
      throw Error_Device("Failed to open ppdev.");
