// $Id: AvrStargate.C,v 1.1.4.1 2007/01/26 06:04:51 pipeng Exp $
...
TAvrStargate::WriteProgramMemoryPage()
{
  ...
  xpl_util_gettimeofday(&t_start_wr, NULL);
  t_wait.tv_sec = 0;
  ...
  xpl_util_gettimeofday(&t_start_poll, NULL);
  timeradd(&t_start_poll, &t_wait, &t_timeout);

  /* Wait */
  do {
    xpl_util_gettimeofday(&t_end, NULL);
    if (poll_data) {
