 ...
 *
 * $Id: FramerM.nc,v 1.1.2.2 2007/04/26 21:59:59 njain Exp $
 */
    ...
    case RXSTATE_INFO:	   //Handle incomming packet now that we are synced to frame
      if (gRxByteCnt > HDLC_MTU) {	  //Packet too long 
		...
		//fetch the received CRC
		  ...
		  usRcvdCRC = (usRcvdCRC << 8) | (gpRxBuf[fRemapRxPos(gRxByteCnt-2)] & 0xff);
#ifdef MICA2MSGTYPE
//DEBUG - FORCE CRC OK
#ifndef NEELUM_FIXES
		  gRxRunningCRC= usRcvdCRC; /* narayan took this out must check CRC*/
#endif /* NEELUM_FIXES */
#endif /* MICA2MSGTYPE */
		  if (usRcvdCRC == gRxRunningCRC) {
            ...
            gMsgRcvTbl[gRxHeadIndex].Length = 0;
            gMsgRcvTbl[gRxHeadIndex].Token = 0;
			gRxState = RXSTATE_NOSYNC; /* added by narayan */
			gRxByteCnt = gRxRunningCRC = 0;
			break;
          	}
		...
		gRxByteCnt = gRxRunningCRC = 0;
			...
			if (gRxByteCnt >= 2) {
				gRxRunningCRC = crcByte(gRxRunningCRC,gpRxBuf[fRemapRxPos(gRxByteCnt-2)]);	//start computing CRC-need remap
			}
			gRxByteCnt++; 
			}
		break;
	...
	data = data ^ 0x20;
        gpRxBuf[fRemapRxPos(gRxByteCnt)] = data;
	if (gRxByteCnt >= 2) {
	  gRxRunningCRC = crcByte(gRxRunningCRC,gpRxBuf[fRemapRxPos(gRxByteCnt-2)]);
	}
	gRxByteCnt++;
	gRxState = RXSTATE_INFO;
      }
