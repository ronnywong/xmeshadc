 ...
 *
 * $Id: MTS410EEPROMM.nc,v 1.1.4.2 2007/04/27 05:45:10 njain Exp $
 */
...
implementation
{
  ...
  command result_t MTS410EEPROM.writePacket[uint8_t id](uint16_t startaddr, char in_length, char* in_data, char in_flags) 
  {
  ...
  command result_t MTS410EEPROM.readPacket[uint8_t id](uint16_t startaddr, char in_length, char in_flags)
  {
	  state=READ_START;
	  device_addr=id;
	  eeprom_addr=startaddr;
  ...
  event result_t I2C.sendEndDone()
  {
	  if(state==WRITE_END)
	  {
		  ...
		  state=WRITE_START;
		  signal MTS410EEPROM.writePacketDone[device_addr](1);
		  return SUCCESS;
	  }
	  if(state==READ_END)
	  {
		  ...
		  signal MTS410EEPROM.readPacketDone[device_addr](index, buffer);
		  return SUCCESS;
	  }
	  return SUCCESS;
  }
  ...
  default event result_t MTS410EEPROM.readPacketDone[uint8_t id](char length, char* data)
  {
	return SUCCESS;
	}
  default event result_t MTS410EEPROM.writePacketDone[uint8_t id](bool result)
  {
	return SUCCESS;
	}
}
